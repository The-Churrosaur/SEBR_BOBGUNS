...
namespace MWI2_CustomEnergyWeapons.Config{
	...
	public class MWI2_GatlingTurretLogic : MWI2_WeaponLogic { }

	[MyEntityComponentDescriptor(typeof(MyObjectBuilder_LargeMissileTurret), true)]
	public class MWI2_LargeMissileTurretLogic : MWI2_WeaponLogic { }

	[MyEntityComponentDescriptor(typeof(MyObjectBuilder_InteriorTurret), true)]
	public class MWI2_InteriorTurretLogic : MWI2_WeaponLogic { }
	
	[MyEntityComponentDescriptor(typeof(MyObjectBuilder_SmallGatlingGun), true)]
	public class MWI2_SmallGatlingGunLogic : MWI2_WeaponLogic { }
	
	[MyEntityComponentDescriptor(typeof(MyObjectBuilder_SmallMissileLauncher), true)]
	public class MWI2_SmallMissileLauncherLogic : MWI2_WeaponLogic { }
	
	[MyEntityComponentDescriptor(typeof(MyObjectBuilder_SmallMissileLauncherReload), true)]
	public class MWI2_SmallMissileLauncherReloadLogic : MWI2_WeaponLogic { }
	
	public class MWI2_WeaponLogic : MyGameLogicComponent{
				...
				foreach(var offset in Settings.BarrelSubpartOffsets){
					...
					if (Settings.UseHitParticles == true && hitForParticle == true && IsDedicated == false){
						...
						if(HitParticleCount[offset] >= Settings.UseParticleAfterRayCount){
							...
							if(MyParticlesManager.TryCreateParticleEffect(Settings.ParticleName, ref hitParticleMatrix, ref particleHitPosition, uint.MaxValue, out effect) == true){
			...
			foreach(var offset in Settings.BarrelSubpartOffsets){
				...
				if(Settings.UseBarrelParticles == true && IsDedicated == false){
					...
					if(BarrelParticles.ContainsKey(offset) == false){
						...
						if(MyParticlesManager.TryCreateParticleEffect(Settings.BarrelParticleName, ref offsetMatrix, ref fromCoords, uint.MaxValue, out effect) == true){
